(function ($) {
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/eead-morphing-layouts.default', WidgetMorphingLayoutHandler);
    });
    ...
    var WidgetMorphingLayoutHandler = function ($scope, $) {
        ...
        class MorphingLayouts {
            ...
            getAnimeObj(targetStr) {
                ...
                if (targetStr === 'path') {
                    if (this.DOM.morph.hover_path == 'yes') {
                        morphanimeOpts.d = this.isActive ? this.DOM.content_arr[0][0] : this.DOM.content_arr[0][1];
                    } else {
                        morphanimeOpts.d = this.isActive ? this.DOM.content_arr[0] : this.DOM.content_arr[0];
                    }
                    if (this.DOM.morph.hover_path != 'yes') {
                        morphanimeOpts.loop = true;
                        morphanimeOpts.fill = {
                            value: this.DOM.morph.fill.color,
                            duration: 500,
                            easing: this.DOM.morph.fill.easing
                        };
                        morphanimeOpts.direction = 'alternate';
                    }
                }

                if (targetStr === 'svg' || targetStr === 'image') {
                    morphanimeOpts.elasticity = this.CONFIG.animation[targetStr].elasticity;
                    morphanimeOpts.scaleX = this.isActive ? this.CONFIG.animation[targetStr].hover_scaleX : this.CONFIG.animation[targetStr].scaleX;
                    morphanimeOpts.scaleY = this.isActive ? this.CONFIG.animation[targetStr].hover_scaleY : this.CONFIG.animation[targetStr].scaleY;
                    morphanimeOpts.translateX = this.isActive ? this.CONFIG.animation[targetStr].hover_translateX : this.CONFIG.animation[targetStr].translateX;
                    morphanimeOpts.translateY = this.isActive ? this.CONFIG.animation[targetStr].hover_translateY : this.CONFIG.animation[targetStr].translateY;
                    morphanimeOpts.rotate = this.isActive ? this.CONFIG.animation[targetStr].hover_rotate : this.CONFIG.animation[targetStr].rotate;
                }
                morphanime.remove(target);
                return morphanimeOpts;
            }
            initShapeLoop(pos, path, conn, connAlt, color, duration, easing) {
                pos = pos || 0;
                morphanime.remove(path);
                morphanime({
                    targets: path,
                    easing: 'linear',
            ...
            initShapeEl() {
                morphanime.remove(this.DOM.svg);
                morphanime({
            ...
            animate() {
                morphanime(this.getAnimeObj('path'));
                morphanime(this.getAnimeObj('svg'));
                morphanime(this.getAnimeObj('image'));
            }
        }
