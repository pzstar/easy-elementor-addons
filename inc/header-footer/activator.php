...
class Activator {
    ...
    protected $post_type = 'eead_template';
    ...
    public static function template_ids(){
        $cached = wp_cache_get( 'eead_template_ids' );
		if ( false !== $cached ) {
			return $cached;
        }
        ...
        if($instance->header_template != null){
			\EEAD\Utils::render_elementor_content_css($instance->header_template);
		}

		if($instance->footer_template != null){
			\EEAD\Utils::render_elementor_content_css($instance->footer_template);
		}

        wp_cache_set( 'eead_template_ids', $ids );
        return $ids;
    }
