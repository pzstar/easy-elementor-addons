    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        Notation = ModuleHandler.extend({
            ...
            onElementChange: function (prop) {
                if (prop.indexOf('eead_notation_') !== -1) {
                    this.run();
                }
            },

            settings: function (key) {
                return this.getElementSettings('eead_notation_' + key);
            },
            ...
            run: function () {
                ...
                $list.forEach(element => {
                    ...
                    if (element.eead_notation_select_type == 'widget') {
                    ...
                    if (element.eead_notation_select_type == 'custom') {
                        var customSelector = element.eead_notation_custom_selector;

                        if (element.eead_notation_custom_selector && customSelector.length > 1) {
                            $selectElement = '[data-id="' + $elementID + '"] ' + ' ' + customSelector;
                        } else {
                            $selectElement = '.-eead-empty';
                        }
                    }

                    if (element.eead_notation_type == 'bracket') {
                        bracketOn = element.eead_notation_bracket_on;
                    ...
                    if ($selectElement.length > 0) {
                        ...
                        options.type = element.eead_notation_type;
                        options.color = element.eead_notation_color || '#f23427';
                        options.animationDuration = element.eead_notation_anim_duration.size || 800;
                        options.strokeWidth = element.eead_notation_stroke_width.size || 1;
                        options.rtl = rtl;
