...
var engine = (function () {
  ...
  function play() {
    if (!raf && (!isDocumentHidden() || !morphanime.suspendWhenDocumentHidden) && activeInstances.length > 0) {
  ...
  function handleVisibilityChange() {
    if (!morphanime.suspendWhenDocumentHidden) { return; }
...
function morphanime(params) {
  ...
  function resetTime() {
    startTime = 0;
    lastTime = adjustTime(instance.currentTime) * (1 / morphanime.speed);
  }
  ...
  instance.tick = function(t) {
    ...
    setInstanceProgress((now + (lastTime - startTime)) * morphanime.speed);
  };
...
morphanime.version = '3.2.1';
morphanime.speed = 1;
// TODO:#review: naming, documentation
morphanime.suspendWhenDocumentHidden = true;
morphanime.running = activeInstances;
morphanime.remove = removeTargetsFromActiveInstances;
morphanime.get = getOriginalTargetValue;
morphanime.set = setTargetsValue;
morphanime.convertPx = convertPxToUnit;
morphanime.path = getPath;
morphanime.setDashoffset = setDashoffset;
morphanime.stagger = stagger;
morphanime.timeline = timeline;
morphanime.easing = parseEasings;
morphanime.penner = penner;
morphanime.random = function (min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; };

//module.exports = morphanime;
...
;(function(window) {
	...
	Line.prototype.options = {
		...
		// 		easing: animation easing (morphanimejs easing. To see all possible values console morphanimejs.easings).
	...
	Line.prototype.animate = function(settings) {
		...
		if( this.options.animation.direction === 'TopBottom' || this.options.animation.direction === 'BottomTop' || this.options.animation.direction === 'CenterV' ) {
			morphanimeProps.scaleY = this.rendered ? [1, 0] : [0, 1];
		}
		else {
			morphanimeProps.scaleX = this.rendered ? [1, 0] : [0, 1];
		}
		...
		var self = this;
		morphanimeProps.complete = function() {
